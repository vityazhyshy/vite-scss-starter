...
1. Установите Node.js версии `20.20.2+`.
   Рекомендуется использовать `nvm`.
   Если в проекте есть файл `.nvmrc`, достаточно выполнить:
...
Если всё настроено правильно, локальный сервер будет доступен:

- по адресу `http://localhost:5173/`
- и по локальному IP в вашей сети, например `http://192.168.0.10:5173/`
...
Во время production build итоговые HTML:

- разворачивают все `@@include(...)`
- перемещаются в корень `dist`
- автоматически форматируются через Prettier
...
Sass в проекте уже переведен на современный API, поэтому старое предупреждение про `legacy JS API` больше не должно появляться при сборке.
...
<a id="-линтинг-форматирование-и-проверки-перед-коммитом"></a>
...
Если нужно прогнать полную проверку перед релизом одной командой, используйте:
...
Если после сборки нужен еще и отчет по размеру итоговых файлов, используйте:
...
8. Перед релизом или публикацией прогонять:

```bash
npm run build:check
```

<a id="-что-важно-знать"></a>
...
- `npm run dev` и `npm run preview` доступны и по `localhost`, и по локальному IP.
- Две production-сборки одновременно запускать нельзя: сборщик теперь специально блокирует второй процесс, чтобы не повредить `dist`.
- В README и других публичных файлах проекта не должно быть локальных путей, паролей, токенов, ключей и других чувствительных данных.
